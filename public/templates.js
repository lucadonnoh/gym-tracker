...
export function renderDayButton(day) {
    let daysAgoText = '';
...
export function renderHistoryItem(session) {
...
export function renderSessionSummary(totalVolume, exerciseCount, totalPRs) {
    return `
...
export function renderSessionDetailExercise(ex, stats) {
    const sets = ex.sets || [];
    ...
    const prBadges = [];
    if (prs.volume)
        prBadges.push('<span class="pr-badge pr-volume">Vol PR</span>');
    if (prs.setVolume)
        prBadges.push('<span class="pr-badge pr-set">Set PR</span>');
    if (prs.weight)
        prBadges.push('<span class="pr-badge pr-weight">1RM</span>');
    if (prs.reps)
        prBadges.push('<span class="pr-badge pr-reps">Reps PR</span>');
...
export function renderManageExercise(ex) {
    return `
...
export function renderVolumeDisplay(currentVolume, lastVolume) {
...
export function renderSetRow(exercise, expected, logged, lastSets) {
...
function renderDropsetRow(exercise, expected, lastSets) {
...
export function renderExtraSetRow(exercise, logged) {
    return `
...
export function formatTimer(elapsed) {
...
export function renderWeeklyGoalModal(currentGoal) {
    const options = [1, 2, 3, 4, 5, 6, 7].map(n => `<button class="goal-option ${n === currentGoal ? 'selected' : ''}" onclick="app.setWeeklyGoal(${n})">${n}x/week</button>`).join('');
    return `
