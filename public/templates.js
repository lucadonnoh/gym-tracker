...
function renderSessionSummary(totalVolume, exerciseCount, totalPRs) {
  return `
...
function renderSessionDetailExercise(ex, stats) {
  const sets = ex.sets || [];
...
function renderManageExercise(ex) {
  return `
...
function renderVolumeDisplay(currentVolume, lastVolume) {
  if (currentVolume <= 0) {
...
function renderSetRow(exercise, expected, logged, lastSets) {
...
function renderDropsetRow(exercise, expected, lastSets) {
  ...
  const dropsetData = [];
  const repsArray = typeof expected.reps === "string" ? expected.reps.split("-").map((r) => parseInt(r) || 10) : [expected.reps];
  for (let i = 0; i < (expected.dropsetParts || 0); i++) {
    ...
    dropsetData.push({
      ...
      logged: subLogged?.weight !== null && subLogged?.weight !== void 0,
...
function renderExtraSetRow(exercise, logged) {
  return `
...
function renderSummaryStats(stats) {
  ...
  const weekDaysHtml = dayLabels.map((label, i) => {
    ...
    const isToday = (/* @__PURE__ */ new Date()).getDay() === jsDay;
...
function renderWeeklyGoalModal(currentGoal) {
  const options = [1, 2, 3, 4, 5, 6, 7].map(
    (n) => `<button class="goal-option ${n === currentGoal ? "selected" : ""}" onclick="app.setWeeklyGoal(${n})">${n}x/week</button>`
  ).join("");
  return `
...
const LOADING_HTML = '<p style="text-align:center;color:#999;padding:2rem;">Loading...</p>';
export {
  LOADING_HTML,
  formatDuration,
  formatTimer,
  ...
  renderExtraSetRow,
  renderHistoryItem,
  renderManageExercise,
  renderSessionDetailExercise,
  renderSessionSummary,
  renderSetRow,
  renderSummaryStats,
  renderVolumeDisplay,
  renderWeeklyGoalModal
};
